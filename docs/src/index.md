...
- **SymPy import fails**: Ensure `PythonCall` is installed in Julia and `sympy` is installed in the Python used by PythonCall. Check with `PythonCall.pyimport("sys").executable`.
